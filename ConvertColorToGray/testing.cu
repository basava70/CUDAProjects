...
__global__ void colorConvertToGrey(unsigned char *rgb, unsigned char *grey,
                                   int rows, int cols) {
...
size_t loadImageFile(unsigned char *grey_image, const std::string &input_file,
                     int *rows, int *cols);

void outputImage(const std::string &output_file, unsigned char *grey_image,
                 int rows, int cols);
...
int main(int argc, char **argv) {
  ...
  unsigned char *h_grey_image,
      *d_grey_image; // host and device's grey data array pointers
  ...
  const size_t total_pixels =
      loadImageFile(h_grey_image, input_file, &rows, &cols);

  // allocate memory of host's grey data array
  h_grey_image =
      (unsigned char *)malloc(sizeof(unsigned char *) * total_pixels);
  ...
  cudaMemcpy(d_rgb_image, h_rgb_image,
             sizeof(unsigned char) * total_pixels * CHANNELS,
  ...
  // execute cuda kernel
  colorConvertToGrey<<<dimGrid, dimBlock>>>(d_rgb_image, d_grey_image, rows,
                                            cols);

  // copy computed gray data array from device to host
  cudaMemcpy(h_grey_image, d_grey_image, sizeof(unsigned char) * total_pixels,
